...
def oauth2_dance(consumer_key, consumer_secret, bearer_filename=None):
    """
    Perform the OAuth2 dance to transform a consumer key and secret into a
    bearer token.

    If a bearer_filename is given, the bearer token will be written to
    the file.
    """
    twitter = Twitter(
    ...
    bearer_token = json.loads(twitter.oauth2.token(grant_type="client_credentials"))[
        "access_token"
    ]
    if bearer_filename:
        write_bearer_token_file(bearer_filename, token)
    return bearer_token
...
def oauth_dance(
    app_name, consumer_key, consumer_secret, oauth_filename=None, open_browser=False
):
    """
    ...
    If a `oauth_filename` is given, the oauth tokens will be written to
    the file.
    ...
    if oauth_filename:
        write_token_file(oauth_filename, oauth_token, oauth_token_secret)
        print()
        print(
