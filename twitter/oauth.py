...
class OAuth(Auth):
    ...
    def encode_params(self, base_url, method, params):
        ...
        key = (
            self.consumer_secret + "&" + urllib_parse.quote(self.token_secret, safe="~")
        )

        message = "&".join(
            urllib_parse.quote(i, safe="~")
            for i in [method.upper(), base_url, enc_params]
        )
        ...
        return (
            ...
            + "&"
            + "oauth_signature="
            + urllib_parse.quote(signature, safe="~")
        )
...
def get_bearer_token(consumer_key, consumer_secret):
    """
    Get a bearer token for use with the application-only authentication
    scheme.
    """
    ...
    return json.loads(handle.read().decode("utf-8"))["access_token"]
