...
from .oauth2 import OAuth2, read_bearer_token_file
from .oauth_dance import oauth_dance
from . import ansi
from .util import smrt_input, printNicely, align_text

DEFAULT_CONSUMER_KEY = "uS6hO2sV6tDKIOeVjhnFnQ"
DEFAULT_CONSUMER_SECRET = "MEYTOS97VvlHX7K1rwHPEqVpTSqZ71HtvoK4sVuYk"
...
OPTIONS = {
    ...
    "config_filename": os.environ.get("HOME", os.environ.get("USERPROFILE", ""))
    + os.sep
    + ".twitter",
    "oauth_filename": os.environ.get("HOME", os.environ.get("USERPROFILE", ""))
    + os.sep
    + ".twitter/oauth",
    "consumer_filename": os.environ.get("HOME", os.environ.get("USERPROFILE", ""))
    + os.sep
    + ".twitter/consumer",
    "bearer_filename": os.environ.get("HOME", os.environ.get("USERPROFILE", ""))
    + os.sep
    + ".twitter/bearer",
    "api_version": "1.1",
    "open_browser": False,
...
class AnsiStatusFormatter(object):
    ...
    def __call__(self, status, options):
        ...
        return "%s%s% 16s%s %s " % (
            get_time_string(status, options),
            ansiFormatter.cmdColour(colour),
            status["user"]["screen_name"],
...
class SearchAction(Action):
    def __call__(self, twitter, options):
        ...
        if not (options["extra_args"] and options["extra_args"][0]):
            raise TwitterError(
                "You need to specify something to search enclosed in single/double quotes"
            )
        query_string = options["extra_args"][0]

        results = twitter.search.tweets(q=query_string, count=options["length"])[
            "statuses"
        ]
        ...
        return list(
            reversed(
                twitter.statuses.user_timeline(
                    screen_name=screen_name, count=options["length"]
                )
            )
        )
...
class RepliesAction(StatusAction):
    def getStatuses(self, twitter, options):
        return list(
            reversed(twitter.statuses.mentions_timeline(count=options["length"]))
        )
    ...
    def __call__(self, twitter, options):
        ...
        for resource in resources:
            for method in resources[resource]:
                ...
                print(
                    "Remaining API requests for %s: %s / %s"
                    % (method, remaining, limit)
                )
                print(
                    "Next reset in %ss (%s)\n"
                    % (int(reset - time.time()), time.asctime(time.localtime(reset)))
                )


class ReplAction(Action):
    def __call__(self, twitter, options):
        upload = Twitter(auth=twitter.auth, domain="upload.twitter.com")
        printNicely(
...
def main(args=sys.argv[1:]):
    ...
    consumer_filename = os.path.expanduser(options["consumer_filename"])
    bearer_filename = os.path.expanduser(options["bearer_filename"])
    consumer_key, consumer_secret = read_token_file(consumer_filename)

    if options["api_version"] == "2":
        if options["action"] == "authorize" or not os.path.exists(bearer_filename):
            bearer_token = oauth2_dance(
                consumer_key=consumer_key,
                consumer_secret=consumer_secret,
                bearer_filename=bearer_filename,
            )
        else:
            bearer_token = read_bearer_token_file(bearer_filename)
        ...
        raise ValueError("Unknown API version: %s" % options["api_version"])
